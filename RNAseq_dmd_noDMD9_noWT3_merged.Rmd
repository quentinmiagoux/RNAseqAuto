...
```{r functions}
datatable_fun <- function(x){x %>% mutate_if(is.numeric, ~format(., scientific = T, digits = 2)) %>% datatable(options = list(
  pageLength=10, scrollX='400px'), filter = 'top', rownames = F, escape = F)
}
```
...
```{r transform data}
expr <- read.xlsx("gene_expression.xlsx")
expr2 <- expr %>% dplyr::select(gene_symbol,read_count_DMD10_1:read_count_WT9_5)
expr2[,-1] <- expr2[,-1] %>% mutate_all(as.numeric) %>% mutate_all(round)
expr3 <- expr2 %>% dplyr::select(gene_symbol,read_count_DMD10_1:read_count_WT9_5)

rownames(expr3) <- expr2$gene_symbol

expr3 = expr3[,-1]

colnames(expr3) <- colnames(expr3) %>% str_remove("read_count_")

expr3 <- expr3[, str_detect(colnames(expr3),"DMD|WT")] 


ord <- order(as.integer(sub("^.*_", "", colnames(expr3))))

expr3 <- expr3[, ord]
colnames(expr3) <- colnames(expr3) %>% str_replace("DNM2_", "DNMII")

colnames(expr3) <- colnames(expr3) %>% sub("[0-9]+_", "_", .) %>% str_remove("_")
colnames(expr3) <- paste0(colnames(expr3),"_",rep(c(1,2,3)))

expr3 <- expr3[,-which(colnames(expr3) %>% str_detect('DMD9|WT3'))]

coldata <- data.frame(samples = colnames(expr3))
coldata$condition = sub("[0-9].*$", "", coldata$samples)
coldata$condition2 = if_else(coldata$condition == "WT","WT","Dystrophy")
coldata <- mutate(coldata, condition3 = str_split(samples,"_", simplify = T ) %>% .[,1])

info <- read.xlsx("RNAseq_proteomics_info.xlsx")
...
dds <- DESeqDataSetFromMatrix(
  countData = expr3,
  colData   = coldata,
  design    =  ~  Reprogrammation + Age + sex + condition2)

dds  <- collapseReplicates(dds, dds$condition3, dds$samples)

dds <- dds[rowSums(counts(dds)) > 10,]   # petit filtre
dds <- DESeq(dds)
res <- results(dds, contrast = c("condition2","Dystrophy","WT"))
rld <- vst(dds, blind=T)

finalDE_padj05 <- res %>% data.frame %>% dplyr::filter(abs(log2FoldChange) > log2(2), padj < 0.05, baseMean>20)  %>% rownames_to_column(var="symbol")
finalDE <- res %>% data.frame %>% dplyr::filter(baseMean>20)  %>% rownames_to_column(var="symbol")

```


```{r}
gene.df <- bitr(finalDE_padj05$symbol, fromType = "SYMBOL",
        toType = c("SYMBOL", "ENTREZID"),
        OrgDb = org.Hs.eg.db,drop = F)

univ.gene.df <- bitr(finalDE$symbol, fromType = "SYMBOL",
        toType = c("SYMBOL", "ENTREZID"),
        OrgDb = org.Hs.eg.db,drop = F)

finalDE_symbol <- finalDE$log2FoldChange
names(finalDE_symbol) <- finalDE$symbol
finalDE_symbol <- finalDE_symbol[order(finalDE_symbol, decreasing = T)]
...
finalDE_entrez <- finalDE_entrez[order(finalDE_entrez$log2FoldChange, decreasing = T),]


gene.df_down <- finalDE_entrez %>% dplyr::filter(log2FoldChange < -log2(2), padj < 0.05) %>% pull(ENTREZID)
gene.df_up <- finalDE_entrez %>% dplyr::filter(log2FoldChange > log2(2), padj < 0.05) %>% pull(ENTREZID)
...
pcaData <- pcaData %>% mutate(samples = str_split(samples,"_", simplify = T ) %>% .[,1])
...
topVar <- head(order(rowVars(mat), decreasing = TRUE), 500)  # 200–1000 selon goût
mat_top <- mat[topVar, ]
...
C3_t2g <- msigdbr(species = "Homo sapiens", category = "H") %>% 
  dplyr::select(gs_name, gene_symbol)


em2 <- GSEA(finalDE_symbol, TERM2GENE = C3_t2g, pvalueCutoff = .5, pAdjustMethod = "fdr")

to_plot <- em2@result %>% filter(p.adjust <= 0.05) %>% dplyr::select(ID, NES, pvalue,p.adjust)

to_plot <- to_plot %>% dplyr::arrange(NES)

to_plot %>% ggplot(aes(fill=p.adjust, y=NES, x=ID)) +  geom_bar(position="stack", stat="identity", color="black") + theme_bw(12) + theme(axis.text.x=element_text(face="bold",color="black",angle=45,hjust=1),plot.margin = margin(.5, .5 , .5 , 6 , "cm"), legend.text = element_text(face = "bold"),legend.title = element_text(face = "bold")) + xlab("") + ylab("NES")  + scale_fill_gradient2(low="red", mid="blue", high = "red")


ggplot() + 
  geom_bar(data=to_plot, 
           aes(x=NES, y=reorder(ID, -p.adjust), fill=p.adjust),
           stat="identity", width=0.7,
           position=position_dodge(0.8))+
  scale_fill_gradient(low="red", high="#FFE6E6") +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 0, hjust = 1, size = 10, color = "black", face = "bold"),
        axis.text.y = element_text(size = 10, color = "black", face = "bold"))+
  theme(legend.position = "bottom",
        legend.text = element_text(size = 12), # Augmenter la taille du texte de légende
        legend.title = element_text(size = 14, face = "bold")) + # Agrandir le titre de la légende
  ylab("")
...
```{r}
gocc_up <- enrichGO(gene          = gene.df_up,
...
```{r}
gocc_down <- enrichGO(gene          = gene.df_down,
                universe      = univ.gene.df$ENTREZID,
                OrgDb         = org.Hs.eg.db,
                ont           = "CC",
                pAdjustMethod = "fdr",
...
gomf_up <- enrichGO(gene          = gene.df_up,
                universe      = univ.gene.df$ENTREZID,
                OrgDb         = org.Hs.eg.db,
                ont           = "MF",
                pAdjustMethod = "BH",
...
gomf_down <- enrichGO(gene          = gene.df_down,
                universe      = univ.gene.df$ENTREZID,
                OrgDb         = org.Hs.eg.db,
                ont           = "MF",
                pAdjustMethod = "BH",
...
```{r}
gobp_up <- enrichGO(gene          = gene.df_up,
                universe      = univ.gene.df$ENTREZID,
                OrgDb         = org.Hs.eg.db,
                ont           = "BP",
                pAdjustMethod = "fdr",
...
```{r}
gobp_down <- enrichGO(gene          = gene.df_down,
                universe      = univ.gene.df$ENTREZID,
                OrgDb         = org.Hs.eg.db,
                ont           = "BP",
                pAdjustMethod = "fdr",
...
```{r}
egocc <- gseGO(geneList     = gsea_gene,
              OrgDb        = org.Hs.eg.db,
              ont          = "CC",
              keyType = "ENTREZID",
              pAdjustMethod = "fdr",
...
test <- any(egocc@result$p.adjust < 0.05)
...
```{r}
egomf <- gseGO(geneList     = gsea_gene,
              OrgDb        = org.Hs.eg.db,
              ont          = "MF",
              pAdjustMethod = "fdr",
              pvalueCutoff = 0.05,
...
if(!is.null(egomf)){
test <- any(egomf@result$p.adjust < 0.05)
...
```{r}
egobp <- gseGO(geneList     = gsea_gene,
              OrgDb        = org.Hs.eg.db,
              ont          = "BP",
              pAdjustMethod = "fdr",
              pvalueCutoff = 0.05,
...
if(!is.null(egobp)){
test <- any(egobp@result$p.adjust < 0.05)
...
```{r}
kk_up <- enrichKEGG(gene   =  gene.df_up,
                organism = "hsa",
                keyType = "kegg",
                universe = univ.gene.df$ENTREZID,
                pvalueCutoff = 0.05,
...
```{r}
kk_down <- enrichKEGG(gene   =  gene.df_down,
                organism = "hsa",
                keyType = "kegg",
                universe = univ.gene.df$ENTREZID,
                pvalueCutoff = 0.05,
...
if(!is.null(kkg)){
test <- any(kkg@result$p.adjust < 0.05)
...
```{r}
wpres_down <- enrichWP(gene.df_down, 
                  organism = "Homo sapiens", 
                  universe = univ.gene.df$ENTREZID,
                  pvalueCutoff = 0.05,
                  pAdjustMethod = "fdr",
...
if(!is.null(wpresg)){
test <- any(wpresg@result$p.adjust < 0.05)
...
```{r}
reactome_up <- enrichPathway(gene=gene.df_up, 
...
```{r}
reactome_down <- enrichPathway(gene=gene.df_down, 
                          universe = finalDE_entrez$ENTREZID,
                          pAdjustMethod = "fdr", 
                          minGSSize = 10,
                          maxGSSize = 500,
...
```{r}
reactome_gsea <- gsePathway(gsea_gene,
                          pAdjustMethod = "fdr",
                          maxGSSize  = 200, 
                          minGSSize = 10,
                          pvalueCutoff = 0.5,
...
test <- any(reactome_gsea@result$p.adjust < 0.05)
