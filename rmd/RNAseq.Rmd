...
```{r venn-diagram, fig.height=8, fig.width=8}
  cols <- setNames(brewer.pal(max(3, length(gene_sets[1:5])), "Set2")[seq_along(gene_sets)], names(gene_sets))
  fit <- venn(gene_sets[1:5])
...
all_intersections <- map_dfr(2:length(gene_sets), function(k) {
  ...
  map_dfr(cmb, function(sel) {
    ...
    tibble(
      selected_comparisons = paste(sel, collapse = " ∩ "),
      n_selected = k,
