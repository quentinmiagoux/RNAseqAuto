...
fc_threshold <- if (!is.null(params$lfc_threshold)) params$lfc_threshold else 2
...
build_disease_table <- function(df, disease) {
  ...
  df %>%
    transmute(
      gene = symbol,
      !!paste0("logFC_", disease) := log2FoldChange,
...
gene_disease_summary <- gene_disease_summary %>%
  select(
    ...
    all_of(paste0("pass_", target_diseases)),
    Count
  )
...
htmltools::tags$p(
  "You can also download the pre-generated workbook:",
  htmltools::tags$a(
    href = "gene_disease_logFC_summary.xlsx",
    "gene_disease_logFC_summary.xlsx",
    download = NA
  )
)
```
