...
coldata$condition2 <- case_when(
  select_group(params$group_a, coldata$condition) ~ params$group_a,
  select_group(params$group_b, coldata$condition) ~ params$group_b,
  TRUE ~ NA_character_
)
...
dds <- DESeq(dds)
res <- results(dds, contrast = c("condition2", params$group_a, params$group_b))
...
ggplot() +
  ...
  theme_classic() +
  theme(
    axis.text.x = element_text(angle = 0, hjust = 1, size = 10, color = "black", face = "bold"),
