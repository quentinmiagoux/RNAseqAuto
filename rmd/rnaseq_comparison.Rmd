...
percentVar_top <- round(100 * attr(pcaData_top, "percentVar"))

pcaData_top <- pcaData_top %>%
  mutate(samples = str_split(samples, "_", simplify = TRUE) %>% .[, 1])
```
...
ggplot(pcaData_all, aes(PC1, PC2, color = samples)) +
...
```{r PCA-top-500}
ggplot(pcaData_top, aes(PC1, PC2, color = samples)) +
  geom_point(size = 5, alpha = 0.8) +
  scale_shape_manual(values = 16:22) +
  scale_color_viridis_d(option = "turbo") +
  xlab(paste0("PC1: ", percentVar_top[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar_top[2], "% variance")) +
  coord_fixed(ratio = 4 / 3) +
  theme_bw()
```
