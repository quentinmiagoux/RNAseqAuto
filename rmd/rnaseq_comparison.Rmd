...
analysis_params <- tibble::tibble(
  parameter = c("run_tag", "fold_change (lfc)", "padj", "expr_xlsx", "coldata_xlsx", "output_root"),
  value = c(
    params$run_tag,
...
```{r transform-data}
cat("Log2FoldChange: ", log2(params$lfc), "\n")
cat("BaseMean: ", params$bm, "\n")
cat("lfcSE: ", params$lfcse, "\n")
cat("padj: ", params$padj, "\n")
cat("lfc: ", params$lfc_threshold, "\n")
...
finalDE_padj05 <- res %>%
  data.frame() %>%
  dplyr::filter(abs(log2FoldChange) > log2(params$lfc), padj < params$padj, baseMean > params$bm, lfcSE < params$lfcse) %>%
  rownames_to_column(var = "symbol")
finalDE <- res %>%
  data.frame() %>%
  dplyr::filter(baseMean > params$bm) %>%
  rownames_to_column(var = "symbol")
```
...
gene.df_down <- finalDE_entrez %>%
  dplyr::filter(log2FoldChange < - log2(params$lfc), padj < params$padj) %>%
  pull(ENTREZID)
...
finalDE_VP <- finalDE
keyvals <- ifelse(
    finalDE_VP$log2FoldChange < - log2(params$lfc) & finalDE_VP$padj < params$padj & finalDE_VP$baseMean > params$bm, 'royalblue',
      ifelse(finalDE_VP$log2FoldChange > log2(params$lfc) & finalDE_VP$padj < params$padj & finalDE_VP$baseMean > params$bm , 'red',
        'grey'))
...
EnhancedVolcano(finalDE_VP, 
                ...
                title = "",
                subtitle = "",
                caption = paste0("Total = ", nrow(finalDE_VP), " variables, ",  "DEGs = ", nrow(finalDE_padj05), " corrected p-value (FDR) < ", params$padj, " & ",  bquote("|FC| > 2 ")),
                x="log2FoldChange", 
                y= "padj",
                pCutoff = 5e-2,
                FCcutoff = log2(params$lfc),
                pointSize = 3.0,
                colCustom = keyvals,
                labSize = 5.0,
                labCol = 'black',
...
if (is.null(run_tag) || !nzchar(run_tag)) {
  run_tag <- paste0(
    "lfc", gsub("\\.", "p", as.character(params$lfc)),
    "_padj", gsub("\\.", "p", as.character(params$padj)),
    "_bm", gsub("\\.", "p", as.character(params$bm)),
    "_lfct", gsub("\\.", "p", as.character(params$lfc_threshold)),
    "_lfcse", gsub("\\.", "p", as.character(params$lfcse))
  )
}

comparison_output_dir <- file.path(params$output_root, params$disease_folder)
dir.create(comparison_output_dir, recursive = TRUE, showWarnings = FALSE)

deg_output_file <- file.path(
  comparison_output_dir,
  paste0("DEGs_", params$comparison, "_", run_tag, ".xlsx")
)
openxlsx::write.xlsx(finalDE_padj05, deg_output_file, overwrite = TRUE)
