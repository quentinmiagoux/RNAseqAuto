...
finalDE_padj05 <- res %>%
  data.frame() %>%
  dplyr::filter(abs(log2FoldChange) > log2(params$lfc), padj < 0.05, baseMean > params$bm, lfcSE < params$lfcse) %>%
...
gene.df_down <- finalDE_entrez %>%
  dplyr::filter(log2FoldChange < - log2(params$lfc), padj < 0.05) %>%
  pull(ENTREZID)
...
finalDE_VP <- finalDE
keyvals <- ifelse(
    finalDE_VP$log2FoldChange < - log2(params$lfc) & finalDE_VP$padj < .05 & finalDE_VP$baseMean > params$bm, 'royalblue',
      ifelse(finalDE_VP$log2FoldChange > log2(params$lfc) & finalDE_VP$padj <  .05 & finalDE_VP$baseMean > params$bm , 'red',
        'grey'))
